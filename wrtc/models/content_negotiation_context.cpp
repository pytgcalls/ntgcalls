...
#include "content_negotiation_context.hpp"
...
namespace wrtc {
        ...
        for (const auto &id : channelIdOrder) {
            ...
            for (const auto &channel : incomingChannels) {
                if (std::to_string(channel.ssrc) == id) {
                    found = true;
                    auto mappedContent = convertSingalingContentToContentInfo(std::to_string(channel.ssrc), channel, webrtc::RtpTransceiverDirection::kRecvOnly);
    ...
    cricket::ContentInfo ContentNegotiationContext::convertSingalingContentToContentInfo(const std::string &contentId, const MediaContent &content, webrtc::RtpTransceiverDirection direction) {
    ...
    cricket::MediaDescriptionOptions ContentNegotiationContext::getIncomingContentDescription(const MediaContent &content) {
        auto mappedContent = convertSingalingContentToContentInfo(std::to_string(content.ssrc), content, webrtc::RtpTransceiverDirection::kSendOnly);
